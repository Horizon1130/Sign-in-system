function navBar(strData){
	...
	if(typeof(strData) == "string"){
		var result = JSON.parse(strData); //部分用户解析出来的是字符串，转换一下
		data = result.menus;
	}else{
		data = strData.menus;
	}	
		...
		if(data[i].children != undefined && data[i].children.length > 0){
			...
			for(var j=0;j<data[i].children.length;j++){
				if(data[i].children[j].target == "_blank"){
					...
					ulHtml += '<dd><a name="menus" href="javascript:;" data-url="'+data[i].children[j].href+'?resourceId='+ data[i].children[j].resourceId +'">';
				}
