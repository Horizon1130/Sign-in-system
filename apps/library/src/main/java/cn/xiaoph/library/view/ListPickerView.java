...
public class ListPickerView extends View {
    ...
    private void performSelect() {
        if (mSelectListener != null) {
            JSONObject item = mDataList.get(mCurrentSelected);
            mSelectListener.onSelect(item);
        }
    }

    public void setData(List<JSONObject> datas) {
        mDataList = datas;
    ...
    public void setSelected(String mSelectItem) {
        for (int i = 0; i < mDataList.size(); i++) {
            String title = mDataList.get(i).getString("title");
            if (title.equals(mSelectItem)) {
                setSelected(i);
                break;
            }
        }
    ...
    private void moveHeadToTail() {
        if (loop) {
            JSONObject item = mDataList.get(0);
            mDataList.remove(0);
            mDataList.add(item);
        }
    }
    ...
    private void moveTailToHead() {
        if (loop) {
            JSONObject item = mDataList.get(mDataList.size() - 1);
    ...
    private void init() {
        ...
        nPaint.setColor(getResources().getColor(R.color.text2));
    }
    ...
    private void drawData(Canvas canvas) {
        ...
        JSONObject item = mDataList.get(mCurrentSelected);
        canvas.drawText(item.getString("title"), x, baseline, mPaint);
        // 绘制上方data
    ...
    private void drawOtherText(Canvas canvas, int position, int type) {
        ...
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));
        JSONObject item = mDataList.get(mCurrentSelected + type * position);
        canvas.drawText(item.getString("title"),
